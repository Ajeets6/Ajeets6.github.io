---
title: ""
pagetitle: "Ajeet Singh - About"
page-layout: full
toc: false
sidebar: false
---

::::: {.grid .hero}

:::: {.g-col-12 .g-col-md-8 .d-flex .flex-column .justify-content-center}

<h1 class="hero-title display-1 fw-bold">Ajeet Singh</h1>
<h2 class="hero-subtitle">Building <span class="highlight purple">Intelligent Systems</span> & <span class="highlight blue">Digital Experiences</span></h2>

::: {.lead .mb-4}
I am a developer passionate about **LLMs**, **Computer Vision**, and **High-Performance Computing**.
I turn complex data into actionable insights and build tools that bridge the gap between AI and reality.
:::

<div>
  <a href="https://www.linkedin.com/in/ajeetsingh6" class="btn-social" target="_blank"><i class="bi bi-linkedin"></i> LinkedIn</a>
  <a href="https://github.com/Ajeets6" class="btn-social" target="_blank"><i class="bi bi-github"></i> Github</a>
  <a href="mailto:your.email@example.com" class="btn-social"><i class="bi bi-envelope"></i> Contact</a>
</div>

::::

:::: {.g-col-12 .g-col-md-4}

::: {.profile-img-container}
![](profile.png)
:::

::::

:::::

::::: {.column-page-inset}

<h2 class="section-heading">Technical Arsenal</h2>

:::: {.skills-grid}

::: {.skill-category .accent-blue}
<div class="category-icon"><i class="bi bi-cpu"></i></div>
<h3 class="category-title">Core Tech</h3>
<div class="skill-chips">
<span class="skill-chip">Python</span>
<span class="skill-chip">C++</span>
<span class="skill-chip">Rust</span>
<span class="skill-chip">JavaScript</span>
</div>
:::

::: {.skill-category .accent-purple}
<div class="category-icon"><i class="bi bi-robot"></i></div>
<h3 class="category-title">AI & ML</h3>
<div class="skill-chips">
<span class="skill-chip">PyTorch</span>
<span class="skill-chip">TensorFlow</span>
<span class="skill-chip">LLMs / RAG</span>
<span class="skill-chip">Computer Vision</span>
<span class="skill-chip">HuggingFace</span>
</div>
:::

::: {.skill-category .accent-orange}
<div class="category-icon"><i class="bi bi-window-stack"></i></div>
<h3 class="category-title">Development</h3>
<div class="skill-chips">
<span class="skill-chip">Docker</span>
<span class="skill-chip">Kubernetes</span>
<span class="skill-chip">FastAPI</span>
<span class="skill-chip">React</span>
<span class="skill-chip">Git</span>
</div>
:::

::: {.skill-category .accent-green}
<div class="category-icon"><i class="bi bi-bar-chart-fill"></i></div>
<h3 class="category-title">Data Science</h3>
<div class="skill-chips">
<span class="skill-chip">Pandas</span>
<span class="skill-chip">NumPy</span>
<span class="skill-chip">Scikit-learn</span>
<span class="skill-chip">Visualization</span>
</div>
:::

::::

<h2 class="section-heading">Selected Projects</h2>

:::: {.project-grid}

::: {.project-card .position-relative}
::: {.project-img-container}
![](posts/multi-modal-rag/rag.png)
:::
::: {.project-content}
[Multimodal Chatbot](posts/multi-modal-rag/rag.md){.stretched-link .text-decoration-none .h3 .d-block .fw-bold}

::: {.project-desc}
PDF Chatbot with local LLMs, extracting text and images for context-aware answers.
:::
:::
:::

::: {.project-card .position-relative}
::: {.project-img-container}
![](posts/dashboard-llm/dashboard1.PNG)
:::
::: {.project-content}
[DashboardLLM](posts/dashboard-llm/dash.md){.stretched-link .text-decoration-none .h3 .d-block .fw-bold}

::: {.project-desc}
Generates interactive charts from CSV data using LLMs and Vega-Lite.
:::
:::
:::

::: {.project-card .position-relative}
::: {.project-content}
[CHIP-8 Emulator](posts/chip8-raylib/chip8.qmd){.stretched-link .text-decoration-none .h3 .d-block .fw-bold}

::: {.project-desc}
CHIP-8 emulator built with Raylib and compiled to WebAssembly for the browser.
:::
:::
:::

::::

<h2 class="section-heading">Journey</h2>

:::: {.experience-grid}

::: {.experience-card .accent-red}

<div class="card-header">
<h3 class="role-title">Senior Data Scientist</h3>
<div class="company">Tech Company Inc.</div>
<div class="period">2023 - Present</div>
</div>

<div class="card-content">
<ul>
<li>Leading the development of Multi-Modal RAG systems for enterprise knowledge retrieval.</li>
<li>Optimized inference latency for LLM deployment by 40%.</li>
<li>Mentoring junior developers and establishing best practices for MLOps.</li>
</ul>
</div>

:::

::: {.experience-card .accent-pink}

<div class="card-header">
<h3 class="role-title">Software Engineer</h3>
<div class="company">Software Solutions Ltd.</div>
<div class="period">2021 - 2023</div>
</div>

<div class="card-content">
<ul>
<li>Developed high-performance backend services using Python and C++.</li>
<li>Implemented real-time dashboarding solutions for data visualization.</li>
<li>Collaborated with cross-functional teams to deliver scalable web applications.</li>
</ul>
</div>

:::

::::



:::::
